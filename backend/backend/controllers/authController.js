...
const register = async (req, res, next) => {
  try {
    ...
    if (role === 'admin') {
      ...
      // Check staff limit for this company's plan
      const planCheck = await pool.query(`
        SELECT p.max_staff,
          (SELECT COUNT(*) FROM users WHERE company_id = $1 AND role = 'staff' AND is_active = TRUE) AS staff_count
        FROM subscriptions s JOIN plans p ON p.id = s.plan_id
        WHERE s.company_id = $1
      `, [companyId]);
      if (planCheck.rows.length) {
        const { max_staff, staff_count } = planCheck.rows[0];
        if (max_staff !== -1 && parseInt(staff_count) >= parseInt(max_staff)) {
          return res.status(402).json({
            message: `This company has reached its staff limit (${max_staff} members). Ask your admin to upgrade the plan.`,
