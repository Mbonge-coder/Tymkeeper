...
const getMySessions = async (req, res, next) => {
  try {
    ...
    if (period === 'today') {
      dateFilter = ` AND s.start_time >= CURRENT_DATE`;
    } else if (from || to) {
...
const getMyStats = async (req, res, next) => {
  try {
    ...
    const result = await pool.query(`
      SELECT
        COALESCE(SUM(CASE WHEN start_time >= CURRENT_DATE THEN work_seconds END), 0) AS today_seconds,
        COALESCE(SUM(CASE WHEN start_time >= DATE_TRUNC('week', CURRENT_DATE) THEN work_seconds END), 0) AS week_seconds,
